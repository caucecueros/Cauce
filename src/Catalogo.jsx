...
// ─── TRANSFORMER: filas CSV → estructura de items ────────────────────────────
  ...
  return filas
    ...
    .map(f => {
      ...
      if (tieneVariantes) {
        ...
        return { id: Number(f.Id), nombre: f.Nombre, variantes };
      }
      return { id: Number(f.Id), nombre: f.Nombre, img: f.Img };
    });
}

// ─── HOOK: carga una hoja del sheet ─────────────────────────────────────────
...
const TarjetaProductoTerminado = ({ item, onVerZoom, coloresUI }) => {
  ...
  // Si cambia el item (ej. recarga), resetear selección
  useEffect(() => {
        ...
        {Object.entries(SECCIONES_CONFIG).map(([id, sec], i) => (
          <div
            ...
            style={{
              ...
              animation: `fadeSlideUp 0.5s ${i * 0.1}s ease both`,
              opacity: 0,
            }}
          >
