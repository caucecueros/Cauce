...
const estilosGlobales = `
  ...
  * { box-sizing: border-box; }
  ...
  .btn-hover { transition: opacity 0.2s, transform 0.2s !important; }
  .btn-hover:hover { opacity: 0.85; transform: translateY(-1px); }
  .icono-footer { transition: opacity 0.2s, transform 0.2s; }
  .icono-footer:hover { opacity: 1 !important; transform: translateY(-2px); }
  .bloque-catalogo { transition: transform 0.3s, box-shadow 0.3s; }
  .bloque-catalogo:hover { transform: translateY(-4px); box-shadow: 0 20px 40px rgba(0,0,0,0.4) !important; }
  .bloque-catalogo:hover .overlay-texto { background-color: rgba(0,0,0,0.65) !important; }
`;
...
function GuiaTalles({ onCerrar }) {
  return (
    <div style={estiloOverlayGuia} onClick={onCerrar}>
      <div style={estiloModalGuia} onClick={e => e.stopPropagation()}>
        <button onClick={onCerrar} style={estiloBotonCerrarGuia}><X size={20} /></button>
...
function Navbar({ carrito, onAbrirCarrito, colores }) {
  const navigate = useNavigate();
  const location = useLocation();

  const enCatalogo = location.pathname.startsWith('/catalogo');
  const enSeccion = location.pathname === '/catalogo/cinturones' || location.pathname === '/catalogo/brazaletes';
  const nombreSeccion = location.pathname === '/catalogo/cinturones' ? 'CINTURONES' : 'BRAZALETES';
    ...
    <nav style={estiloNavbar(colores)}>
      ...
      {enCatalogo && (
        <div style={estiloBreadcrumb}>
          ...
          {enSeccion ? (
            <>
              <span style={estiloMigaInactiva} onClick={() => navigate('/catalogo')}>CATÁLOGO</span>
              <ChevronRight size={12} style={{ opacity: 0.4 }} />
              <span style={estiloMigaActiva}>{nombreSeccion}</span>
            </>
          ) : (
...
function Home({ onVerCatalogo, colores }) {
  return (
    <>
      <header style={estiloHero}>
        ...
        <button className="btn-hover" onClick={onVerCatalogo} style={{ ...estiloBotonHero(colores), animation: 'fadeSlideUp 0.6s 0.5s ease both', opacity: 0 }}>
          VER CATÁLOGO
        </button>
      </header>
      ...
      <section style={{ padding: 'clamp(60px, 10vw, 100px) clamp(16px, 5vw, 40px)', backgroundColor: '#F4F1EE' }}>
        <div style={{ maxWidth: '1500px', margin: '0 auto' }}>
          ...
          <div style={{ textAlign: 'center' }}>
            <button className="btn-hover" onClick={onVerCatalogo} style={estiloBotonSeccion(colores)}>
      ...
      <footer style={{ padding: 'clamp(40px, 8vw, 60px) 20px', textAlign: 'center', backgroundColor: colores.marronOscuro, color: colores.fondoClaro }}>
        <div style={{ display: 'flex', justifyContent: 'center', gap: '36px', marginBottom: '16px' }}>
          <div className="icono-footer" onClick={() => window.open(`https://instagram.com/${INSTAGRAM_USER}`, '_blank')} style={{ cursor: 'pointer', opacity: 0.8 }}>
            <Instagram size={26} color={colores.blancoCrema} />
          </div>
          <div className="icono-footer" onClick={() => window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent('Hola! Quería consultar sobre sus productos 😊')}`, '_blank')} style={{ cursor: 'pointer', opacity: 0.8 }}>
...
function App() {
  ...
  const enviarPorWhatsApp = () => {
    ...
    const mensaje = `Hola! Quiero hacer un pedido en Cauce Cueros 🤎\n\n📦 *Mi pedido:*\n${lineas}\n\nQuedo esperando confirmación, gracias!`;
  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh', fontFamily: 'serif', backgroundColor: '#1a0f0b' }}>
        ...
        <Navbar carrito={carrito} onAbrirCarrito={() => setCarritoAbierto(true)} colores={colores} />

        <div style={{ animation: 'fadeSlideUp 0.5s ease both' }}>
          <Routes>
            <Route path="/" element={<Home onVerCatalogo={() => navigate('/catalogo')} colores={colores} />} />
            ...
            <Route path="/catalogo/:seccion" element={
              <div style={{ backgroundImage: "url('/pantalla5.jpg')", backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed', minHeight: '100vh' }}>
                <Catalogo onAgregar={agregarAlCarrito} />
              </div>
            } />
            ...
            <div style={{ ...estiloPanelCarrito, animation: 'panelSlideIn 0.35s ease both' }}>
              <div style={estiloHeaderCarrito}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                  {carrito.length > 0 && (
                    <button onClick={() => setGuiaAbierta(true)} style={{ background: 'none', border: 'none', color: colores.plataletra, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px', fontSize: '0.65rem', letterSpacing: '1px' }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          {!esBraz ? (
                            ...
                            <div>
                              ...
                              <input type="number" min="12" max="25" placeholder="ej: 17" value={item.medidaMuneca} onChange={e => actualizarMedida(index, e.target.value)} style={estiloInputMedida} />
                            </div>
                          )}
                        </div>
...
const estiloHero = { padding: 'clamp(60px, 12vh, 140px) clamp(16px, 5vw, 40px)', textAlign: 'center', maxWidth: '100%', backgroundImage: "url('/pantalla5.jpg')", backgroundSize: 'cover', backgroundPosition: 'center' };
const estiloBotonHero = (c) => ({ padding: 'clamp(14px, 2vw, 18px) clamp(28px, 5vw, 45px)', backgroundColor: c.marronOscuro, color: 'white', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '4px', cursor: 'pointer', fontSize: 'clamp(0.7rem, 1.5vw, 0.85rem)', letterSpacing: '2px', fontWeight: 'bold' });
const estiloBotonSeccion = (c) => ({ padding: 'clamp(14px, 2vw, 22px) clamp(28px, 5vw, 50px)', backgroundColor: c.marronOscuro, color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: 'clamp(0.75rem, 1.5vw, 0.9rem)', letterSpacing: '3px', fontWeight: 'bold', boxShadow: '0 10px 20px rgba(65, 37, 28, 0.3)' });
const estiloContador = { position: 'absolute', top: '-4px', right: '-4px', backgroundColor: '#e74c3c', color: 'white', borderRadius: '50%', padding: '1px 5px', fontSize: '0.6rem', fontWeight: 'bold', border: '1px solid #fff' };
const estiloPanelCarrito = { position: 'fixed', top: 0, right: 0, width: 'min(420px, 100vw)', height: '100%', backgroundColor: '#41251c', color: 'white', padding: 'clamp(20px, 4vw, 40px) clamp(16px, 4vw, 30px)', boxShadow: '-5px 0 20px rgba(0,0,0,0.5)', fontFamily: 'serif', display: 'flex', flexDirection: 'column', zIndex: 200, overflowY: 'auto' };
const estiloHeaderCarrito = { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', borderBottom: '1px solid rgba(188, 193, 195, 0.2)', paddingBottom: '18px', flexShrink: 0 };
const estiloItemCarrito = { display: 'flex', alignItems: 'flex-start', gap: '12px', borderBottom: '1px solid rgba(188, 193, 195, 0.08)', paddingBottom: '16px' };
const estiloImagenCarrito = { width: 'clamp(50px, 10vw, 65px)', height: 'clamp(50px, 10vw, 65px)', objectFit: 'cover', borderRadius: '4px', flexShrink: 0 };
const estiloBotonTalle = { padding: '4px 9px', border: '1px solid rgba(188,193,195,0.4)', borderRadius: '3px', cursor: 'pointer', fontSize: '0.62rem', fontWeight: 'bold', letterSpacing: '0.5px', transition: 'all 0.15s' };
const estiloInputMedida = { width: '72px', padding: '5px 8px', backgroundColor: 'rgba(255,255,255,0.08)', border: '1px solid rgba(188,193,195,0.35)', borderRadius: '4px', color: '#fff', fontSize: '0.8rem', outline: 'none' };
const estiloBotonEliminar = { background: 'none', border: 'none', color: 'rgba(231,76,60,0.8)', cursor: 'pointer', fontSize: '0.75rem', flexShrink: 0, paddingTop: '2px', transition: 'color 0.15s' };
const estiloBotonFinalizar = { marginTop: '16px', padding: '16px', backgroundColor: '#bcc1c3', color: '#41251c', border: 'none', borderRadius: '4px', fontWeight: 'bold', letterSpacing: '2px', cursor: 'pointer', textTransform: 'uppercase', fontSize: 'clamp(0.7rem, 2vw, 0.85rem)', flexShrink: 0 };
const estiloCajaMaterial = { backgroundColor: 'white', borderRadius: '4px', overflow: 'hidden', boxShadow: '0 10px 30px rgba(0,0,0,0.08)', textAlign: 'left' };
const estiloImagenMaterial = { height: 'clamp(180px, 25vw, 300px)', backgroundSize: 'cover', backgroundPosition: 'center' };
const estiloOverlayGuia = { position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.85)', zIndex: 500, display: 'flex', justifyContent: 'center', alignItems: 'center', animation: 'fadeIn 0.25s ease', padding: '20px' };
const estiloModalGuia = { backgroundColor: '#2a1a14', color: '#fff', borderRadius: '8px', padding: 'clamp(24px, 5vw, 40px)', maxWidth: '460px', width: '100%', position: 'relative', border: '1px solid rgba(255,255,255,0.12)', animation: 'fadeSlideUp 0.3s ease both', maxHeight: '90vh', overflowY: 'auto' };
const estiloBotonCerrarGuia = { position: 'absolute', top: '14px', right: '14px', background: 'none', border: 'none', color: '#fff', cursor: 'pointer', opacity: 0.6 };
const estiloSubtituloGuia = { fontSize: '0.72rem', letterSpacing: '2px', opacity: 0.6, marginBottom: '10px', fontFamily: 'sans-serif' };
const estiloTabla = { width: '100%', borderCollapse: 'collapse', fontFamily: 'sans-serif', fontSize: '0.82rem' };
const estiloCeldaHeader = { textAlign: 'left', padding: '8px 10px', borderBottom: '1px solid rgba(255,255,255,0.15)', fontSize: '0.65rem', letterSpacing: '1px', opacity: 0.55, fontWeight: 'bold' };
const estiloCelda = { padding: '8px 10px', borderBottom: '1px solid rgba(255,255,255,0.05)' };
